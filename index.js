...
const translations = translationsFolder
  ? fs.readdirSync(translationsFolder).reduce((result, translationFilePath) => {
      console.log(translationFilePath);
...
const lines = Object.keys(yamlMeasures).map(measureFamilyName => {
  const labels = {
    ...getLabels(translations, "families", measureFamilyName),
    ...yamlMeasures[measureFamilyName].labels
  };
  const standardUnit = yamlMeasures[measureFamilyName].standard;
  const units = Object.keys(yamlMeasures[measureFamilyName].units).map(
    unitCode => ({
      code: unitCode,
      labels: {
        ...getLabels(translations, "units", unitCode),
        ...yamlMeasures[measureFamilyName].units[unitCode].labels
      },
      convert_from_standard: yamlMeasures[measureFamilyName].units[unitCode].convert.map(
        convertObject => {
